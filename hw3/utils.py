...
ID = 204664544  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array):
    ...
    # ====== YOUR CODE: ======
    accuracy_val = np.sum(y == y_pred) / y.size
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    # calculate the norm squared of both vector lists
    # reshape for broadcasting. x1 norms will be broadcast to columns and x2 to rows
    x1_squ = np.sum(x1 ** 2, axis=-1).reshape(-1, 1)
    x2_squ = np.sum(x2 ** 2, axis=-1).reshape(1, -1)

    # create matrix of all inner products
    x1x2 = x1 @ x2.T

    # putting it all together
    dists_squ = x1_squ + x2_squ - 2*x1x2

    # account for floating point precision error before sqrt
    dists_squ[dists_squ < 0] = 0

    # sqrt to get actual distance
    dists = np.sqrt(dists_squ)
    # ========================
...
def load_data_set(clf_type: str):
    ...
    hw_path = pathlib.Path(__file__).parent.absolute()
