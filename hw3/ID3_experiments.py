...
def find_best_pruning_m(train_dataset: np.array, m_choices, num_folds=5):
    ...
    for i, m in enumerate(m_choices):
        ...
        # ====== YOUR CODE: ======
        m_accuracies = []  # container for all fold accuracies for current m

        # iterate folds
        kf = KFold(n_splits=num_folds, shuffle=True, random_state=ID)
        for train_split, valid_split in create_train_validation_split(train_dataset, kf):
            x_train, y_train, x_valid, y_valid = get_dataset_split(train_split, valid_split, target_attribute)
            model.fit(x_train, y_train)  # fit
            y_pred = model.predict(x_valid)  # predict
            m_accuracies.append(accuracy(y_valid, y_pred))  # get accuracy

        print(f'done {m=}')
        print(f'mean acc: {np.mean(m_accuracies)}')

        accuracies.append(m_accuracies)  # remember accuracies
        # ========================
...
def cross_validation_experiment(plot_graph=True):
    ...
    best_m = None
    accuracies = []
    m_choices = [i for i in range(10, 41, 5)]  # 15 - 105 in jumps of 15
    num_folds = 5
    if len(m_choices) < 5:
