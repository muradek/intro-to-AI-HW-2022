...
class ID3:
    def __init__(self, label_names: list, min_for_pruning=0, target_attribute='diagnosis'):
        ...
        self.__label_names_no_target = self.label_names[:]  # copy list
        self.__label_names_no_target.remove(self.target_attribute)  # remove target
    ...
    def entropy(rows: np.ndarray, labels: np.ndarray):
        ...
        p = np.array(list(counts.values()), dtype=np.int) / len(rows)

        # although p will never truly be 0 since `counts` only includes existing labels, this can still occur due to
        # floating point precision errors. replace all 0 values with 1 since this will yield the same result without
        # calling `np.log` with a value of 0.
        p[p == 0] = 1
    ...
    def info_gain(self, left, left_labels, right, right_labels, current_uncertainty):
        ...
        info_gain_value = 0.0
        # ====== YOUR CODE: ======
        num_left_samples, num_right_samples = len(left), len(right)
        num_samples = num_left_samples + num_right_samples

        left_weighted_impurity = (num_left_samples / num_samples) * self.entropy(left, left_labels)
        right_weighted_impurity = (num_right_samples / num_samples) * self.entropy(right, right_labels)

        info_gain_value = current_uncertainty - left_weighted_impurity - right_weighted_impurity
        # ========================
    ...
    def partition(self, rows, labels, question: Question, current_uncertainty):
        ...
        true_rows, true_labels = np.array(true_rows), np.array(true_labels)
        false_rows, false_labels = np.array(false_rows), np.array(false_labels)
    ...
    def find_best_split(self, rows, labels):
        ...
        for col_idx in range(rows.shape[-1]):
            # iterate possible mid-values in the column
            unique_values_in_column = np.unique(rows[:, col_idx])
            midways = (unique_values_in_column[:-1] + unique_values_in_column[1:]) / 2
            for v in midways:
                # create column question for current value
                question = Question(column=self.__label_names_no_target[col_idx], column_idx=col_idx, value=v)

                # calculate partition for current question
                gain, true_rows, true_labels, false_rows, false_labels = self.partition(rows, labels, question,
                                                                                        current_uncertainty)

                # if gain is larger, set best values
                if gain >= best_gain:  # use >= to keep maximum index
                    best_gain = gain
    ...
    def build_tree(self, rows, labels):
        ...
        # ====== YOUR CODE: ======
        # if branch is pure or if it has less than the minimum number of samples, make a leaf
        if len(np.unique(labels)) == 1 or len(rows) <= self.min_for_pruning:
            return Leaf(rows, labels)

        # otherwise, split branches and return tree node
        (_, best_question, best_true_rows,
         best_true_labels, best_false_rows, best_false_labels) = self.find_best_split(rows, labels)
    ...
    def predict_sample(self, row, node: DecisionNode or Leaf = None):
        ...
        if isinstance(node, Leaf):
            prediction = max(node.predictions.keys(), key=lambda k: node.predictions[k])
        elif node.question.match(row):  # `match` can handle multiple rows
