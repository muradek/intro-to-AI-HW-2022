...
class BreadthFirstSearchRobot(Robot):
    ...
    def solve(self, maze: MazeProblem, time_limit=float("inf")):
        ...
        while True:
            ...
            # we added this check to account for the case of initial_node being a goal state
            # NOTE: this check should have been done BEFORE the loop
            if maze.is_goal(next_node.state):
                return GraphSearchSolution(final_node=next_node,
                                           solve_time=curr_time() - start_time,
                                           n_node_expanded=n_node_expanded)

            # count newly expanded node for efficiency analysis
            n_node_expanded += 1

            # iterate possible state expansions for current node state
            for new_state, cost in maze.expand_state(next_node.state):
                # check if state is already in some colleciton. if so, skip state
                if new_state in self.queue or new_state in self.close:
                    continue

                # make node object from newly expanded state with given cost
                new_node = Node(new_state, parent=next_node, g_value=next_node.g_value + cost)

                # check if newly expanded node is a goal state
                if maze.is_goal(new_node.state):
                    return GraphSearchSolution(final_node=new_node,
                                               solve_time=curr_time() - start_time,
                                               n_node_expanded=n_node_expanded)

                # if not a solution, add to open and continue
...
class BestFirstSearchRobot(Robot):
    ...
    def solve(self, maze_problem: MazeProblem, time_limit=float("inf"), compute_all_dists=False):
        ...
        while True:
            ...
            # iterate possible state expansions for current node state
            for new_state, cost in maze_problem.expand_state(next_node.state):

                # make node object from newly expanded state with given cost
                new_node = Node(new_state, parent=next_node, g_value=next_node.g_value + cost)

                # calculate new node priority
                new_node_priority = self._calc_node_priority(new_node)

                # if the new state has never been visited, add it to open with new priorities
                if new_state not in self.open and new_state not in self.close:
                    # add new node into priority queue
                    self.open.add(new_node, new_node_priority)

                # compare new state to existing node in open / close
                else:
                    # get existing node from opne / close
                    if new_state in self.open:
                        containing_set = self.open
                    else:
                        containing_set = self.close
                    existing_state_node = containing_set.get_node(new_state)

                    # check if this is a better path to this state
                    if new_node.g_value < existing_state_node.g_value:
                        # found better path to this state
                        # update state node in priority queues
                        containing_set.remove_node(existing_state_node)
                        self.open.add(new_node, new_node_priority)
